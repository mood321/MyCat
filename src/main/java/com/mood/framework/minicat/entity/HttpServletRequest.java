...
public class HttpServletRequest {
    ...
    private String scheme = (MoodCatConfig.HTTP_PORT == 443 ? "https" : "http");
    ...
    public String getParament(String paramName) {
        ...
        if(MultipartFile.class.isAssignableFrom(value.getClass())){
            try {
                return new String(((MultipartFile)value).getFileContext(),MoodCatConfig.ENCODE);
    ...
    private void initParams(){
        params = ParamentLoad.builderParament(queryString);
        if (header.containsKey("Content-Type") && header.get("Content-Type").contains("multipart/form-data")) {
            ...
            Map<String, List<Object>> paramMap = ParamentLoad.buildMultipartParams(data, boundary);
            params = mergeParaMap(params, paramMap);
        } else {
            String postContext = getPostContext();
            if (!StringUtil.isNullOrEmpty(postContext)) {
                Map<String, List<Object>> paramMap = ParamentLoad.builderParament(postContext);
                params = mergeParaMap(params, paramMap);
            }
        }
    }
    ...
    public String getPostContext() {
        try {
            ...
            return new String(data, MoodCatConfig.ENCODE);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public String getSessionId() {
        ...
        if (!cookie.contains(MoodCatConfig.SESSION_ID_FIELD_NAME)) {
            return null;
        }
        String[] cookies = cookie.split(";");
        for (String line : cookies) {
            if (!line.contains(MoodCatConfig.SESSION_ID_FIELD_NAME)) {
                continue;
            }
